    ...
    while (iterate) {
     ...
     # Make sure the new value is in bounds
      ...
      while(!isout) {
	      ...
	      isout = testBounds(par_copy, TP=data$TP, PP=data$PP)
      }
...
function(par,TP,PP)
{
	...
	test = numeric(length(TP))
	test[alphac<=0 | alphac>=1] = 1
	test[alphad<=0 | alphad>=1] = 1
	test[betac<=0 | betac>=1] = 1
	test[betad<=0 | betad>=1] = 1
	test[thetac<=0 | thetac>=1] = 1
	test[thetad<=0 | thetad>=1] = 1
	test[eps<=0 | eps>=1] = 1
	
    res = TRUE
    if(sum(test) > 0) res = FALSE
