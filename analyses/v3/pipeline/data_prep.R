...
T_sp  <- c("bop","peb","peg","pet","prp","sal","soa")
C_sp  <- c("epn","epb","epr","mel","pig","sab")
D_sp  <- c("boj","chr","err","ers","fra","frn","heg","osv","til","cet")
#D_sp  <- c("ers","fra","heg","osv","til","cet")

# Subset species BA and cover type observed
class_dat  <- dat[,c(7:62,65)]
class_dat$sum_tot <- rowSums(dat[,7:62],na.rm=TRUE) 

class_dat$T_prop  <- rowSums(dat[which(names(dat) %in% T_sp)],na.rm=T)
class_dat$C_prop  <- rowSums(dat[which(names(dat) %in% C_sp)],na.rm=T)
class_dat$D_prop  <- rowSums(dat[which(names(dat) %in% D_sp)],na.rm=T)
...
class_fn = function(x) { 
  classPlot = "Unclass"
  if(sum(x,na.rm=T) < 10) classPlot = "T"
  else if(x[2] > 0 & x[3] == 0) classPlot = "C"
  else if(x[3] > 0 & x[2] == 0) classPlot = "D"
  else if (x[2] > 0 & x[3] > 0) classPlot = "M"
  return(classPlot)
}
...
write.table(reshape_dat,file="data/data_categorical.txt")
